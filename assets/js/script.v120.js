...
var TelePrompter = (function() {
  ...
  function initUI() {
    ...
    $(document).on('change', '.font_size', function(event) {
      updateFontSize(true);
    });
    $(document).on('change', '.speed', function(event) {
      updateSpeed(true);
    });
    // overlay
    $(document).on('change', '.ioverlay', function(event) {
      var top =  parseInt($('.ioverlay').val());
      $('.overlay .bottom').css({'top': top});
    });
  ...
  function remoteUpdate(oldConfig, newConfig) {
    ...
    if (oldConfig.fontSize !== newConfig.fontSize) {
      $elm.fontSize.val( newConfig.fontSize);
      updateFontSize(true, true);
    }

    if (oldConfig.pageSpeed !== newConfig.pageSpeed) {
      $elm.speed.val(newConfig.pageSpeed);
  ...
  function updateFontSize(save, skipUpdate) {
    config.fontSize = $elm.fontSize.val();
  ...
  function updateSpeed(save, skipUpdate) {
    config.pageSpeed = $elm.speed.val();
    $('label.speed_label > span').text('(' + $elm.speed.val() + ')');

    if (save) {
      localStorage.setItem('teleprompter_speed', $elm.speed.val());
    }
  ...
  return {
    ...
    setSpeed: function(speed) {
      ...
      $elm.speed.val(parseInt(speed));
      return this;
    },
    setFontSize: function(size) {
      size = Math.min(100, Math.max(12, size));
      $elm.fontSize.val(parseInt(size));
      return this;
    },
